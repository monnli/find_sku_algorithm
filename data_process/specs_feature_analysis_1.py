...
def spu_map_sku(df):
    ...
    result_dict = {
        "query_id": list(), "result_id": list(), "weighted_score": list(), "title_score": list(), "query_url": list(),
        "result_url": list(), "stdCateName": list(), "stdSubCateName": list(), "stdSubCate2Name": list(),
        "query_siteName": list(), "result_siteName": list(), "brandName": list(), "query_specs": list(),
        "result_specs": list(), "success": list()
    }
    for i in range(df.shape[0]):
        ...
        try:
            if spu_to_sku_df.shape[0] > 1:
                ...
                sub_df1 = sub_df1[['skuId', 'canonicalUrl', 'specs', 'siteName']]
                sub_df2 = spu_to_sku_df[spu_to_sku_df['spuId'] == spu_id_2]
                sub_df2 = sub_df2[['skuId', 'canonicalUrl', 'specs', 'siteName']]

                for j in range(sub_df1.shape[0]):
                    sku1 = sub_df1.iloc[j, 0]
                    specs1 = sub_df1.iloc[j, 2]
                    siteName1 = sub_df1.iloc[j, 3]
                    name_list1 = list()
                    color_name1 = ''
                    for g in specs1:
                    ...
                    for k in range(sub_df2.shape[0]):
                        ...
                        result_dict['brandName'] = spu_brand_name
                        result_dict['query_specs'].append(sub_df1.iloc[j, 2])
                        result_dict['result_specs'].append(sub_df2.iloc[k, 2])
                        result_dict['query_siteName'].append(siteName1)
                        result_dict['result_siteName'].append(siteName2)
...
if __name__ == '__main__':
    ...
    df = spu_map_sku(df)
    df.to_excel("coat_jacket_sku_specs所有匹配对_2.xlsx")
